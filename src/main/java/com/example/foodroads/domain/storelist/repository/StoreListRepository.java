...
public interface StoreListRepository extends JpaRepository<StoreList, Long> {
    ...
    @Query("select sl from StoreList sl join StoreListDetail sld on sl = sld.storeList where sl.name like %:keyword% or sl.member.name like %:keyword% or sld.store.name like %:keyword% or sld.description like %:keyword%")
